...
Persona_method_constructor:
    ...
    move $s1, $a1    # $s1 = param[1]
    move $s2, $a2    # $s2 = param[2]
    move $t0, $s1    # t0 = nombre
    # TODO: unsupported TAC op nop (    # Param 0 is reference to 'Self' for Persona)
    li $t8, 0
    add $t1, $s0, $t8    # t1 = self + 0
    sw $t0, 0($t1)
    move $t2, $s2    # t2 = edad
    # TODO: unsupported TAC op nop (    # Param 0 is reference to 'Self' for Persona)
    li $t8, 4
    add $t3, $s0, $t8    # t3 = self + 4
    sw $t2, 0($t3)
...
Persona_method_saludar:
    ...
    la $t0, str0     # t4 = &"Hola "
    li $t8, 0
    add $t1, $s0, $t8    # t5 = self + 0
    lw $t2, 0($t1)
    move $t3, $t2    # t7 = t6
    add $t4, $t0, $t3    # t8 = t4 + t7
...
func_main:
    ...
    move $t3, $t0    # t12 = t9
    move $s0, $t3    # p = t12
    move $a0, $s0    # param[0] = p
    jal Persona_method_saludar   # call Persona_method_saludar()      
    move $t4, $v0    # ret of Persona_method_saludar()
    move $t5, $t4    # t14 = t13
